...
object Main {
  def main(args: Array[String]): Unit = {
    ...
    var (df_train, df_val) = (df(0 until idx, ::), df(idx until df.rows, ::))

    var X_train = df_train(::, 1 to df_train.cols - 2)
    var X_val = df_val(::, 1 to df_val.cols - 2)
